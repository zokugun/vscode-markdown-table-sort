...
import { sortNumber } from './commands/sort-number.js';
import { sortText } from './commands/sort-text.js';
import { ContextServiceManager } from './context/context-service-manager.js';

const CONFIG_KEY = 'markdownTableSort';
...
async function showWhatsNewMessage(version: string) { // {{{
	...
	const result = await vscode.window.showInformationMessage(
		`Markdown Table Sort has been updated to v${version} — check out what's new!`,
		...actions,
	);
...
export async function activate(context: vscode.ExtensionContext): Promise<void> { // {{{
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('markdownTableSort.sortTextAsc', async () => sortText('asc')),
		vscode.commands.registerCommand('markdownTableSort.sortTextDesc', async () => sortText('desc')),
		vscode.commands.registerCommand('markdownTableSort.sortNumberAsc', async () => sortNumber('asc')),
		vscode.commands.registerCommand('markdownTableSort.sortNumberDesc', async () => sortNumber('desc')),
	);
} // }}}
